...
import { repositoryEvidenceExtractorService } from "./repository-evidence-extractor.service.js";
import { buildWorkspacePrompt, WORKSPACE_SCHEMA_HINT } from "./workspace-prompt-builder.js";
...
export class WorkspaceIntelligenceService {
  ...
  async getModule(userId: string, repositoryId: string, moduleType: WorkspaceModuleType): Promise<WorkspaceModuleResponse> {
    ...
    const provenance = { generationSource:"groq" as const, provider:"groq" as const, model:String(data.model), repositoryId, repositoryFullName:`${repo.owner_login}/${repo.name}`, contextSnapshotId:snapshot.id, headSha:snapshot.head_sha, promptVersion:data.prompt_version, contentVersion:data.content_version, generatedAt:data.generated_at, grounded:data.grounded===true, evidenceCoverage:Number(data.evidence_coverage??0), evidenceItemsUsed:Number(payload.evidenceItemsUsed??0), estimatedInputTokens:Number(payload.estimatedInputTokens??data.input_tokens??0), configuredTokenBudget:Number(payload.configuredTokenBudget??0), evidenceTruncated:Boolean(payload.evidenceTruncated), generationDurationMs:Number(payload.generationDurationMs??data.latency_ms??0) };
  ...
  async mentorQuery(userId: string, repositoryId: string, question: string, depth: "beginner"|"standard"|"maintainer", sourceModule?: string): Promise<MentorQueryResponse> {
    ...
    const mentorHash=createHash("sha256").update(question.trim().toLowerCase()).digest("hex");
    const evidencePackage = await this.evidencePackage(userId,knowledge,"mentor",snapshot.id,snapshot.head_sha,{mentorQuestion:question,...(sourceModule?{selectedModule:sourceModule}:{})},mentorHash);
    if(!evidencePackage.evidence.some((item)=>item.type!=="repository_metadata"))return {answer:"There is not enough repository evidence to answer this question.",depth,evidence:[],suggestedQuestions:[],insufficientEvidence:true,fallbackUsed:false};
    this.assertGroqConfiguration();
    const mentorPrompt=JSON.stringify({question,depth,sourceModule,evidencePackage});
    if(mentorPrompt.length>env.WORKSPACE_MAX_PROMPT_CHARS)throw Object.assign(new Error("Mentor evidence exceeds prompt budget"),{code:"EVIDENCE_BUDGET_EXCEEDED"});
    const ai = await aiProviderService.generateJson<Omit<MentorQueryResponse,"fallbackUsed">>({ system: this.systemPrompt(), prompt: mentorPrompt, schemaHint: '{"answer":"repository-grounded answer","depth":"beginner|standard|maintainer","evidence":["existing/path"],"suggestedQuestions":[],"insufficientEvidence":false}' });
  ...
  private async runJob(jobId:string,userId:string,repositoryId:string) {
    try {
      ...
      for(const moduleType of MODULES){ const [stage,percent]=STAGES[moduleType]; await this.progress(jobId,"selecting_evidence",percent-2); await this.progress(jobId,stage,percent); await this.generateModule(jobId,userId,repositoryId,snapshot.id as string,moduleType,context.knowledgePackage); }
  ...
  private async generateModule(jobId:string,userId:string,repositoryId:string,snapshotId:string,moduleType:WorkspaceModuleType,knowledge:RepositoryKnowledgePackage){
    ...
    try {
      ...
      const evidencePackage=await this.evidencePackage(userId,knowledge,moduleType,snapshotId,snapshot?.head_sha??"",{collaboration},"");
      await this.progress(jobId,"compressing_context",STAGES[moduleType][1]);
      if(!evidencePackage.evidence.some((item)=>item.type!=="repository_metadata")){await this.storeInsufficientEvidence(userId,repositoryId,snapshotId,moduleType,knowledge.fullName,snapshot?.head_sha??"",evidencePackage);return;}
      const built=buildWorkspacePrompt(evidencePackage,WORKSPACE_PROMPT_VERSION,MODULE_VERSION);
      if(built.estimatedTokens>evidencePackage.tokenBudget)throw Object.assign(new Error("Evidence package remains above the module token budget"),{code:"EVIDENCE_BUDGET_EXCEEDED"});
      await this.progress(jobId,"preparing_groq_request",STAGES[moduleType][1]); console.info("Workspace module generation started",{repositoryId,repositoryFullName:knowledge.fullName,moduleType,contextSnapshotId:snapshotId,provider:"groq",model:env.AI_DEFAULT_MODEL,promptVersion:WORKSPACE_PROMPT_VERSION,evidenceCount:evidencePackage.evidence.length,promptTokenEstimate:built.estimatedTokens}); console.info("Groq request started",{repositoryId,moduleType});
      const ai=await aiProviderService.generateJson({system:this.systemPrompt(),prompt:built.prompt,schemaHint:WORKSPACE_SCHEMA_HINT}); console.info("Groq request completed",{repositoryId,moduleType,inputTokens:ai.inputTokens,outputTokens:ai.outputTokens});
      await this.progress(jobId,"validating_content",STAGES[moduleType][1]); const grounded=validateGeneratedPayload(ai.data,knowledge,collaboration,evidencePackage); const generatedAt=new Date().toISOString(); const contentHash=semanticContentHash(grounded);
      ...
      const payload={...grounded,repositoryId,repositoryFullName:knowledge.fullName,headSha:snapshot?.head_sha??"",contextSnapshotId:snapshotId,generationSource:"groq",provider:"groq",model:ai.model,promptVersion:WORKSPACE_PROMPT_VERSION,contentVersion:MODULE_VERSION,generatedAt,evidenceItemsUsed:evidencePackage.evidence.length,estimatedInputTokens:built.estimatedTokens,configuredTokenBudget:evidencePackage.tokenBudget,evidenceTruncated:evidencePackage.truncated,generationDurationMs:Date.now()-started};
  ...
  validateConfiguration(){ if(env.AI_ORCHESTRATION_ENABLED)return; if(env.AI_PROVIDER!=="groq")throw new ConflictError("Legacy Workspace generation requires AI_PROVIDER=groq"); if(!env.GROQ_API_KEY)throw new ConflictError("GROQ_API_KEY is required for legacy Workspace generation"); if(!env.AI_DEFAULT_MODEL)throw new ConflictError("AI_DEFAULT_MODEL is required for legacy Workspace generation"); }
  private assertGroqConfiguration(){this.validateConfiguration();}
  private async evidencePackage(userId:string,knowledge:RepositoryKnowledgePackage,moduleType:WorkspaceModuleType,snapshotId:string,headSha:string,context:import("./repository-evidence-extractor.service.js").WorkspaceEvidenceContext,queryHash:string){const {data}=await this.supabase.from("workspace_evidence_packages").select("evidence_payload,head_sha").eq("user_id",userId).eq("repository_id",knowledge.repositoryId).eq("context_snapshot_id",snapshotId).eq("module_type",moduleType).eq("query_hash",queryHash).eq("evidence_version",env.REPOSITORY_EVIDENCE_VERSION).eq("prompt_version",WORKSPACE_PROMPT_VERSION).is("stale_at",null).maybeSingle();if(data?.head_sha===headSha)return data.evidence_payload as import("@openforge/shared").RepositoryEvidencePackage;const evidencePackage=repositoryEvidenceExtractorService.extract(knowledge,moduleType,{contextSnapshotId:snapshotId,headSha},context);await this.cacheEvidencePackage(userId,evidencePackage,queryHash);return evidencePackage;}
  private async cacheEvidencePackage(userId:string,evidencePackage:import("@openforge/shared").RepositoryEvidencePackage,queryHash:string){const {error}=await this.supabase.from("workspace_evidence_packages").upsert({user_id:userId,repository_id:evidencePackage.repositoryId,context_snapshot_id:evidencePackage.contextSnapshotId,head_sha:evidencePackage.headSha,module_type:evidencePackage.moduleType,query_hash:queryHash,evidence_version:env.REPOSITORY_EVIDENCE_VERSION,prompt_version:WORKSPACE_PROMPT_VERSION,token_budget:evidencePackage.tokenBudget,estimated_tokens_used:evidencePackage.estimatedTokensUsed,evidence_payload:evidencePackage,truncated:evidencePackage.truncated,generated_at:evidencePackage.generatedAt,stale_at:null},{onConflict:"user_id,repository_id,context_snapshot_id,module_type,query_hash,evidence_version,prompt_version"});if(error)throw error;}
  private async storeInsufficientEvidence(userId:string,repositoryId:string,snapshotId:string,moduleType:WorkspaceModuleType,fullName:string,headSha:string,evidencePackage:import("@openforge/shared").RepositoryEvidencePackage){const now=new Date().toISOString();const payload={status:"insufficient_evidence",summary:"The repository does not contain enough evidence for this module.",cards:[],repositoryId,repositoryFullName:fullName,contextSnapshotId:snapshotId,headSha,generationSource:"groq",provider:"groq",model:env.AI_DEFAULT_MODEL,promptVersion:WORKSPACE_PROMPT_VERSION,contentVersion:MODULE_VERSION,generatedAt:now,grounded:true,evidenceCoverage:0,evidenceItemsUsed:evidencePackage.evidence.length,estimatedInputTokens:evidencePackage.estimatedTokensUsed,configuredTokenBudget:evidencePackage.tokenBudget,evidenceTruncated:evidencePackage.truncated,generationDurationMs:0};const {error}=await this.supabase.from("workspace_module_content").upsert({user_id:userId,repository_id:repositoryId,context_snapshot_id:snapshotId,module_type:moduleType,content_version:MODULE_VERSION,generation_source:"groq",provider:"groq",model:env.AI_DEFAULT_MODEL,prompt_version:WORKSPACE_PROMPT_VERSION,content_payload:payload,status:"insufficient_evidence",grounded:true,evidence_coverage:0,completed_at:now,last_heartbeat_at:now,generated_at:now},{onConflict:"user_id,repository_id,context_snapshot_id,module_type,content_version"});if(error)throw error;}

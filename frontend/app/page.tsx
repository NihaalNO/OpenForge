          ...
          <div className="space-y-3">
            {["openforge/frontend", "supabase/examples", "vercel/turbo"].map((repo) => (
              <div key={repo} className="rounded-[24px] border border-border bg-background p-4">
                ...
                <p className="mt-2 text-sm text-muted-foreground">Open a workspace with repository context already attached.</p>
              </div>
            ))}
          </div>
        </Card>
        ...
        <div className="grid gap-4 lg:grid-cols-5">
          {["Overview", "Repository map", "Mission", "Mentor prompts", "Quality gate"].map((item) => (
            <Card key={item} className="p-5">
              ...
              <p className="mt-2 text-sm leading-6 text-muted-foreground">Clear, scoped guidance grounded in repository context.</p>
            </Card>
          ))}
        </div>
